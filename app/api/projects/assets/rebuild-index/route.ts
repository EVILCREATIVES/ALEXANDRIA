...
import { saveManifest, fetchManifestDirect, type PageAsset } from "@/app/lib/manifest";
...
function parseAssetPath(pathname: string) {
  // Only match .png files, not .meta.txt or other extensions
  const m = pathname.match(/^projects\/[^/]+\/assets\/p(\d+)\/(p\d+-img\d+)\.png$/);
  if (!m) return null;
      ...
      p.assets = verifiedAssets
        ...
        .map((ba) => {
          ...
          return {
            ...
            tags: prev?.tags,
            negativeTags: prev?.negativeTags,
            trigger: prev?.trigger,
            tagRationale: prev?.tagRationale,
            title: prev?.title,
